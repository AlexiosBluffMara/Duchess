...
// TODO-PRINCIPAL: Edge ML detector review — critical issues:
//   1. No NNAPI delegate. The Vuzix M400 has a Snapdragon XR1 with DSP/NPU, but we
//      only try GpuDelegate. LiteRT's NnApiDelegate would route ops to the DSP for
//      better power efficiency. Add NNAPI as first-choice delegate, GPU as fallback.
//   2. Bitmap.createScaledBitmap() on the hot path allocates a new Bitmap EVERY FRAME.
//      At 10fps that's 10 Bitmap allocations/sec, each triggering GC pressure. Pre-allocate
//      a reusable scaled bitmap and use Canvas.drawBitmap() with a Matrix to scale in-place.
//   3. The LABELS array is hardcoded and must match training data class order. If Elena
//      retrains with a different class order, detections silently mislabel. Load labels
//      from a sidecar labels.txt file (already generated by download-ppe-model.py).
//   4. No temporal voting / smoothing. A single frame detection at confidence 0.36 fires
//      immediately. Construction PPE detection needs temporal consistency — require N
//      consecutive frames with the same violation before escalating. This dramatically
//      reduces false positives from motion blur, occlusion, and lighting changes.
//   5. close() is called manually but there's no guarantee it runs (process kill, ANR).
//      The GpuDelegate holds Vulkan/OpenGL resources. Consider a destructor guard or
//      move to Closeable with use{} pattern at the call site.
//   6. Demo mode cycles deterministically — the same "violation" fires at the same frame
//      count every time. In demos this looks fake. Add randomized timing.
//
// TODO-ML-PROF: Detection model optimization opportunities:
//   - YOLOv8-nano at 640x640 is overkill for PPE detection. Most hard hats occupy >10%
//     of the frame. Test YOLOv8-nano at 320x320 — should halve inference time with
//     minimal accuracy loss on large objects. Run the PPE eval set at both resolutions.
//   - INT8 quantization was applied post-training. For better INT8 accuracy, use
//     Quantization-Aware Training (QAT) during the YOLOv8 training run. Ultralytics
//     supports QAT export. Compare INT8-PTQ vs INT8-QAT on our Construction-PPE eval.
//   - The confidence threshold (0.35) was "tuned" but there's no ROC/PR curve to justify
//     it. Generate precision-recall curves at thresholds [0.1, 0.2, ..., 0.9] and pick
//     the operating point that maximizes F1 for violation classes specifically (we care
//     more about recall on no_hardhat than precision on hardhat).
//   - For TurboQuant/GPTQ comparison: export the same YOLOv8-nano to INT4 via GPTQ and
//     measure accuracy vs INT8. If INT4 is within 1% accuracy, the 2x memory savings
//     matters on the 6GB Vuzix M400.
//   - NMS IoU threshold 0.45 is "standard" but our use case is unusual — PPE items are
//     spatially correlated (vest and hardhat overlap on the same person). Run an ablation
//     on IoU thresholds [0.3, 0.4, 0.5, 0.6] measuring multi-violation detection rate.
